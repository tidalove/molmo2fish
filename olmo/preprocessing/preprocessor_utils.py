...
class TokenizedVisionData:
    ...
    cum_token_pooling_bounds: Optional[Dict[str, np.ndarray]] = None
    "cumulative token pooling boundaries used when training with context parallelism on images and videos"

    cum_image_bounds: Optional[int] = None
    """boundaries of crops in an input"""

    other_data: Optional[Dict[str, np.ndarray]] = None
    """Any extra data that needs to be passed to the model"""

    token_mapping: Optional[np.ndarray] = None
    """Map patch coordinates -> patch ids"""
