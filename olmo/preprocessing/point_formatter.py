...
class PointFormatter:
    ...
    def format_video_tracks(self, point_tracks: List[PointTrack], scale, label, alt_text=None, rng=None,
    ...
    def extract_tracks(self, text, width, height, video_fps, format=None) -> List[PointTrack]:
        raise NotImplementedError()
...
def extract_points(text, image_w, image_h) -> List[Tuple[float, float]]:
    for cls in [UnifiedPointFormatter(), LegacyPointFormatting]:
...
def extract_tracks(text, width, height, video_fps, format=None) -> List[PointTrack]:
    for cls in [UnifiedPointFormatter(), LegacyPointFormatting]:
        points = cls.extract_tracks(text, width, height, video_fps, format)
        if points:
            return points
    return []


def extract_any_points(text, metadata) -> List[Tuple[int, float, float, float]]:
    if "image_size" in metadata:
        w, h = metadata["image_size"]
    elif "image_group_size" in metadata:
        w, h = zip(*metadata["image_group_size"])
    else:
        return []
    points = UnifiedPointFormatter().extract_multi_image_points(text, w, h, extract_ids=True)
    # put example id first
    if "image_group_size" in metadata:
        # Text uses one-based indexing, switch to 0 based
        points = [[int(ix), t-1, x, y] for t, ix, x, y in points]
    else:
        points = [[int(ix), t, x, y] for t, ix, x, y in points]
    if points:
        return points
    return []
...
class UnifiedPointFormatter(PointFormatter):
    ...
    def format_video_tracks(self, point_tracks, scale, label, alt_text=None, rng=None,
                            start_end_only=False, single_point_track=False, from_initial_points=None):
        """
        ...
        if start_end_only:
            point_tracks = self._filter_all_but_start_end(point_tracks)
        # if single_point_track:
        #     point_tracks = self._filter_initial_point(point_tracks)
        if from_initial_points is not None:
            point_tracks = self._filter_for_initial_points(point_tracks, from_initial_points)

        coord_str = self.build_video_track_coordinates(point_tracks, scale)
    ...
    def build_video_track_coordinates(self, point_tracks: List[PointTrack], scale, skip_occluded=True) -> str:
        frame_strings = []
        point_id_to_index = {}
        for point_track in sorted(point_tracks, key=lambda x: x["time"]):
            if not point_track["points"]:
                continue

            points_dict = point_track["points"]
            ...
            time_str = self.format_output_timestamp(parse_timestamp(point_track["time"]))
            frame_strings.append(f"{time_str} {frame_points_str}")
        return self.image_sep.join(frame_strings)

    def _filter_initial_point(self, point_tracks: List[PointTrack]) -> List[PointTrack]:
        filtered = []
        for point_track in point_tracks[1:]:
            if not point_track["points"]:
                continue
            points = point_track["points"]
            if 0 in points:
                filtered.append(dict(point_track, points={0: points[0]}))
        return filtered

    def _filter_for_initial_points(self, point_tracks: List[PointTrack], initial_points) -> List[PointTrack]:
        """
        Filter point_tracks to only include points AFTER initial_points
        Args:
            point_tracks: List of frame data with points
        ...
        for point_track in point_tracks:
            ...
            points_dict = {point_idx: point_track["points"][point_id] for point_id, point_idx in point_id_mapping.items()
                           if point_track["time"] > initial_points[point_idx]["time"] and point_id in point_track["points"]}
            if points_dict:
                # filtered.append(dict(point_track, points=points_dict))
                filtered.append(dict(point_track))
            # filtered.append(points_dict)
        return filtered

    @staticmethod
    def _filter_all_but_start_end(point_tracks: List[PointTrack]) -> List[PointTrack]:
        object_frames = defaultdict(list)
        for i, point_track in enumerate(point_tracks):
            for obj_id, point_info in point_track["points"].items():
                 object_frames[obj_id].append(i)

        # Maps time -> point_track, we might get data for this task due to how the source data
        # works so merge `point_track` with same time here as well
        filtered_point_track = {}
        for i, point_track in enumerate(point_tracks):
            filtered_points = {}
            for obj_id, point_info in point_track["points"].items():
            ...
            if filtered_points:
                time = point_track["time"]
                if time not in filtered_point_track:
                    filtered_point_track[time] = dict(point_track, points={})
                filtered_point_track[time]["points"].update(filtered_points)
        return list(filtered_point_track.values())
...
for backwards-compatibility with some older models
"""
...
@dataclasses.dataclass
class LegacyPointFormatting(PointFormatter):
    ...
    @classmethod
    def extract_tracks(cls, text, width, height, video_fps, format=None):
    ...
    def _format_video_point_track_per_frame(self, point_tracks, scale):
        """Format as: time {t}\n{id: [x, y], ...}"""
        output_parts = []
        ...
        for point_track in point_tracks:
            ...
            frame_points_formatted: str = self._format_video_points_per_frame(point_track["points"], scale)
            if frame_points_formatted:
                time_str = format_time(point_track["time"], "seconds")
                output_parts.append(f"time {time_str}")
                output_parts.append(frame_points_formatted)

        return "\n".join(output_parts)

    def _format_video_point_start_end(self, point_tracks, scale):
    ...
    def _format_video_single_point_track(self, point_tracks, scale):
        ...
        for point_track in point_tracks[1:]:
            ...
            point_info = point_track["points"][0]  # Single point
    ...
    def _format_video_point_track_all_frames_with_occlusion(self, point_tracks, scale, initial_points):
        """Format as time {t}\n{id: [x, y, occluded], ...}"""
        ...
        for point_track in point_tracks:
            ...
            # {point_idx: {"point": [x,y], "occluded": bool}}
            points_dict = {point_idx: point_track["points"][point_id] for point_id, point_idx in point_id_mapping.items()
                           if point_track["frame"] > initial_points[point_idx]["frame"] and point_id in point_track["points"]}
    ...
    def format_video_tracks(self, point_tracks, scale, label, alt_text=None, rng=None,
        ...
        if from_initial_points is not None:
            return self._format_video_point_track_all_frames_with_occlusion(point_tracks, scale, from_initial_points)
        if start_end_only:
            return self._format_video_point_start_end(point_tracks, scale)
        elif single_point_track:
            return self._format_video_single_point_track(point_tracks, scale)
        else:
            return self._format_video_point_track_per_frame(point_tracks, scale)
