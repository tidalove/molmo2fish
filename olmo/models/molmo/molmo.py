...
class MolmoConfig(BaseModelConfig):
    ...
    def build_collator(self, shapes_to_pad_to, pad_mode: str, include_metadata=True) -> MMCollator:
...
class Molmo(ModelBase):
    ...
    def warmup_cache(self, device, cp_enabled=False):
    ...
    def forward(
        ...
        deepstack_image_features: Optional[List[torch.Tensor]] = None
        is_image_patch: Optional[torch.Tensor] = None
        if images is not None:
            vision_backbone_output = self.vision_backbone(images, image_masks, token_pooling)
            if isinstance(vision_backbone_output, list):
                image_features = vision_backbone_output[0]
                deepstack_image_features = vision_backbone_output[1:]
            else:
        ...
        if self.config.bi_directional_attn:
            ...
            if self.config.bi_directional_attn == "within_image":
                # image token can attend to other tokens in the same image
                # image i can attend to image j if i >= j
                if self.config.mm_preprocessor.use_single_crop_start_token:
                    image_start_id = self._low_res_image_start
                else:
                    image_start_id = self._image_start_token_id
                image_starts = input_ids == image_start_id
            ...
            if subsegment_ids is not None:
                ...
                can_attend_bk = can_attend_bk & (example_id[:, None] == example_id[:, :, None])
        ...
        for block_idx, block in enumerate(self.transformer.blocks):
            ...
            if deepstack_image_features is not None and block_idx in range(len(deepstack_image_features)):
                x.view(-1, x.shape[-1])[is_image_patch] += deepstack_image_features[block_idx]
    ...
    def generate(
        ...
        token_pooling: Optional[torch.Tensor] = batch.get("token_pooling")
