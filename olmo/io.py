...
from cached_path.schemes import S3Client, SchemeClient, add_scheme_client, GsClient
...
def copy_dir(
    ...
    if num_threads is None:
        from .util import get_default_thread_count
...
def dir_is_empty(dir: PathOrStr) -> bool:
    ...
    if not is_url(dir):
        ...
        if not os.path.isdir(dir):
            raise ValueError(f"Directory not found or is not a directory: {dir}")
...
def _gcs_get_bytes_range(bucket_name: str, key: str, bytes_start: int, num_bytes: int) -> bytes:
    ...
    return blob.download_as_bytes(
        start=bytes_start,
        end=None if num_bytes is None else bytes_start + num_bytes - 1,
...
def _gcs_list_directory(
    ...
    for match_glob in (
        ...
        for blob in blobs:
            # Note we have to iterate through `blobs` even if `include_files=False` since
            # iterating sets `blobs.prefixes`
            if include_files:
...
class _FixedGsClient(GsClient):
    """GsClient That handles files names with ? in them"""

    def __init__(self, resource: str) -> None:
        super().__init__(resource)
        self.blob = _FixedGsClient.get_gcs_blob(resource)
        self._loaded = False
    ...
    @staticmethod
    def get_gcs_blob(resource: str):
        gcs_client = _get_gcs_client()
        bucket_name, gcs_path = _FixedGsClient.split_gcs_path(resource)
...
def add_cached_path_clients():
    ...
    add_scheme_client(_FixedGsClient)
