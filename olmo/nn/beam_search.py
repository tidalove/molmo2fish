...
class TopKTopPSampler(Sampler):
    """
    A `Sampler` which combines top-k and top-p (nucleus) sampling.

    First applies top-k filtering (if k is set), then applies top-p filtering (if p is set),
    and finally samples from the filtered distribution after re-normalizing.

    When both k and p are specified, top-k is applied first to limit the candidate set,
    then top-p is applied to that reduced set. This is the standard approach used in
    most modern language models.

    Beams are sampled in the default, deterministic way.

    :param k: The number of top choices to be selected from. If None, no top-k filtering is applied.
    :param p: The cumulative probability cutoff threshold. If None, no top-p filtering is applied.
        A higher value of `p` will result in more possible examples to sample from.
    :param temperature: A `temperature` below 1.0 produces a sharper probability distribution and a `temperature`
        above 1.0 produces a flatter probability distribution.
    :param with_replacement: If set to `True`, samples will be selected with replacement.
    """
    ...
    def __init__(
        ...
        if k is None and p is None:
            raise ValueError("At least one of k or p must be specified")
    ...
    def sample_nodes(
        ...
        if not per_node_beam_size <= vocab_size:
            raise ValueError("per_node_beam_size cannot be greater than vocabulary size")
        ...
        # Step 1: Apply top-k filtering if specified
        if self.k is not None:
            if not per_node_beam_size <= self.k <= vocab_size:
                raise ValueError(
                    "k must be a positive integer no less than per_node_beam_size and no greater than vocabulary size"
                )

            # Get top k values and indices
            # shape: (batch_size, k)
            top_k_log_probs, top_k_indices = _log_probs.topk(self.k, dim=-1)

            # Create a tensor for the filtered log probs (rest are masked out)
            filtered_log_probs = torch.full_like(_log_probs, torch.finfo(_log_probs.dtype).min)
            filtered_log_probs.scatter_(-1, top_k_indices, top_k_log_probs)
            _log_probs = filtered_log_probs

        # Step 2: Apply top-p filtering if specified
        if self.p is not None:
            ...
            # Create exclusion mask for tokens beyond cumulative probability p
            # shape: (batch_size, vocab_size)
            exclusion_mask = probabilities_summed >= self.p

            # Include the first index where cumsum >= p by shifting
            exclusion_mask[..., 1:] = exclusion_mask[..., :-1].clone()
            exclusion_mask[..., 0] = False

            # Ensure at least per_node_beam_size options remain
            if not self.with_replacement:
                exclusion_mask[..., :per_node_beam_size] = False

            # Mask out excluded tokens
            log_probs_descending[exclusion_mask] = torch.finfo(_log_probs.dtype).min

            # Convert back to original ordering
            _log_probs = torch.gather(
                log_probs_descending,
                -1,
                sorting_indices.argsort(-1)
            )

        # Step 3: Re-normalize and sample
        # shape: (batch_size, vocab_size)
        normalized_probs = torch.nn.functional.softmax(_log_probs, dim=-1)

        # Sample from the filtered and re-normalized distribution
        # shape: (batch_size, per_node_beam_size)
        sampled_indices = torch.multinomial(
            normalized_probs, per_node_beam_size, replacement=self.with_replacement
        )

        # Gather the log probabilities and return
        # shape: (batch_size, per_node_beam_size)
        selected_log_probs = log_probs.gather(1, sampled_indices)

        return selected_log_probs, sampled_indices, state
...
class RepetitionPenaltyConstraint(Constraint):
    """
    Applies a penalty that scales with the frequency of token appearances.
    Tokens that appear more often receive stronger penalties.

    This implements the repetition penalty as described in the CTRL paper
    (https://arxiv.org/abs/1909.05858), where:
    - If log_prob > 0: divide by penalty
    - If log_prob < 0: multiply by penalty

    :param penalty: The penalty multiplier/divisor. Values > 1.0 discourage repetition.
        Typical values: 1.0 (no penalty) to 2.0 (strong penalty)
    """
    ...
    def init_state(self, batch_size: int) -> ConstraintStateType:
        """Initialize state with empty token frequency counter for each batch."""
        return [[{"token_counts": {}}] for _ in range(batch_size)]
    ...
    def apply(
        ...
        """
        Apply repetition penalty based on token frequency.
        """
        if self.penalty == 1.0:
            return class_log_probabilities

        # Create a copy to avoid modifying the original
        penalized_log_probs = class_log_probabilities.clone()

        for i, batch in enumerate(state):
            for j, beam in enumerate(batch):
                token_counts = beam["token_counts"]

                for token_id, count in token_counts.items():
                    current_log_prob = penalized_log_probs[i, j, token_id]

                    # Apply penalty based on the sign of log probability
                    # This is the standard approach from the CTRL paper
                    if current_log_prob < 0:
                        penalized_log_probs[i, j, token_id] = current_log_prob * self.penalty
                    else:
                        penalized_log_probs[i, j, token_id] = current_log_prob / self.penalty

        return penalized_log_probs
    ...
    def _update_state(
        ...
        """Increment the count for the predicted tokens."""
        for i, batch in enumerate(state):
            for j, beam in enumerate(batch):
                prediction = last_prediction[i, j].item()
                token_counts = beam["token_counts"]
                token_counts[prediction] = token_counts.get(prediction, 0) + 1
        return state


class FrequencyPenaltyConstraint(Constraint):
    """
    Alternative implementation: subtracts (count * penalty) from log probabilities.
    This is simpler and more interpretable than the CTRL-style repetition penalty.

    :param penalty: The penalty value to subtract per occurrence.
        Final penalty = count * penalty
    ...
    def apply(
        ...
        for i, batch in enumerate(state):
            for j, beam in enumerate(batch):
                ...
                for token_id, count in token_counts.items():
                    # Subtract penalty scaled by frequency
                    penalized_log_probs[i, j, token_id] -= (count * self.penalty)
...
class BeamSearch:
    ...
    def _search(
        ...
        frame_time_stamps = state.pop("frame_time_stamps") if "frame_time_stamps" in state else None
        ...
        if (
            ...
            if bmm is None or high_res_indices is  None:
                # Default setting for most beam search operations for Molmo
                return start_predicted_classes.unsqueeze(-1), start_top_log_probabilities
            else:
                return start_predicted_classes.unsqueeze(-1), start_top_log_probabilities, high_res_indices, bmm, frame_time_stamps
        ...
        if bmm is None or high_res_indices is  None:
            ...
            return sorted_all_predictions, sorted_final_scores, high_res_indices, bmm, frame_time_stamps
