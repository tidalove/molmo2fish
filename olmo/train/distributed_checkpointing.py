...
from olmo.torch_util import barrier, get_fs_local_rank, get_global_rank, is_distributed, gc_cuda, get_world_size
    ...
    *,
    ...
    for model_key in model_key_list:
        if f"model.{model_key}" not in metadata.state_dict_metadata:
            if "frame_selection_scorer" in model_key or "frame_selection_score_scaler" in model_key:
                frame_selection_needs_init=True
                break

    if frame_selection_needs_init:
        if model.frame_selection_scorer is not None:
            model.frame_selection_scorer.reset_parameters()
        if model.frame_selection_score_scaler is not None:
    ...
    for model_key in model_key_list:
        if f"model.{model_key}" not in metadata.state_dict_metadata:
            skipped_modules[model_key] = state_dict['model'].pop(model_key)
    ...
    if len(skipped_modules) > 0:
        for key in ["vision_backbone.pad_embed"]:
            if key in skipped_modules:
                ...
                state_dict['model'][key] = skipped_modules.pop(key)

        if any(["temporal_token_scorer" in key for key in skipped_modules]):
            if get_global_rank() == 0:
                print("temporal_token_scorer weights don't exist in state dict, reinitializing")
            model.vision_backbone.image_vit.transformer.temporal_token_scorer.reset_parameters()
            temporal_token_scorer_statedict = model.vision_backbone.image_vit.transformer.temporal_token_scorer.state_dict()

            statedict_new = {}
            for key, item in temporal_token_scorer_statedict.items():
                statedict_new[f"vision_backbone.image_vit.transformer.temporal_token_scorer.{key}"] = item

            state_dict['model'].update(statedict_new)

        if any(["frame_selection" in key for key in skipped_modules]):

            import os
            from transformers import AutoModel
            from olmo.data.dataset import VIDEO_DATA_HOME

            hf_source = model.config.mm_preprocessor.frame_sel_clip_identifier
            ...
            for model_key in skipped_modules.keys():
                if frame_selection_needs_init and "frame_selection_scorer" in model_key or "frame_selection_score_scaler" in model_key:
                    frame_sel_sd[model_key] = skipped_modules[model_key]
                    continue
                if model_key not in frame_sel_sd:
                    raise ValueError(f"Missing key {model_key} not from frame selection model")
            ...
            first_key = [k for k in state_dict['model'].keys()][0]
            frame_selection_to_dtensor = isinstance(state_dict['model'][first_key], DTensor)  # For train set to DTensor. Otherwise, regular Tensors
            if frame_selection_to_dtensor:
                ...
                state_dict['model'].update(frame_sel_sd)
